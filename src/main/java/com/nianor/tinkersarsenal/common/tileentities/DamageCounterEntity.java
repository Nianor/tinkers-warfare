...
public class DamageCounterEntity extends TileEntity implements ITickable{
    ...
    public boolean shouldRefresh(World world, BlockPos pos, IBlockState oldState, IBlockState newState) {
        System.out.println("Should Refresh? "+!newState.getBlock().isAir(newState, world, pos));
        return !newState.getBlock().isAir(newState, world, pos);
    }

    public void damage(int damage) {
        System.out.println("Damage Counter Taking Damage");customTileData.setInteger("damage", customTileData.getInteger("damage")+damage);
    }
