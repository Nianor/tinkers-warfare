...
public class InternalMagazineCore extends RecieverCore {

    public InternalMagazineCore () {
...
*/
    ...
    public float getWeight(ItemStack internalmagazine)
    {
        return MechanicalNBT.from(internalmagazine).weight;
    }

    public float getMech(ItemStack internalmagazine)
    {
        return MechanicalNBT.from(internalmagazine).mech_effectiveness;
    }

    public int getDurability(ItemStack internalmagazine)
    {
        return MechanicalNBT.from(internalmagazine).durability;
    }

    public int getDamage(ItemStack internalmagazine)
    {
        return MechanicalNBT.from(internalmagazine).damage;
    }

    public float getHardness(ItemStack internalmagazine)
    {
        return MechanicalNBT.from(internalmagazine).hardness;
    }

    public void damageItem(ItemStack internalmagazine, EntityLivingBase player, int damage)
    {
        ToolHelper.damageTool(internalmagazine, damage, player);
    }

    @SideOnly(Side.CLIENT)
    public int getCoreColor(ItemStack internalmagazine)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(internalmagazine));
        return materials.get(0).materialTextColor;
    }

    @SideOnly(Side.CLIENT)
    public int getFaceColor(ItemStack internalmagazine)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(internalmagazine));
    ...
    @SideOnly(Side.CLIENT)
    public int getMechColor(ItemStack internalmagazine)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(internalmagazine));
