...
public class AirCooledBarrelCore extends BarrelCore {

    public AirCooledBarrelCore () {
...
*/
    ...
    public float getWeight(ItemStack aircooledbarrel)
    {
        return BarrelNBT.from(aircooledbarrel).weight;
    }

    public float getRifling(ItemStack aircooledbarrel)
    {
        return BarrelNBT.from(aircooledbarrel).rifling;
    }

    public int getDurability(ItemStack aircooledbarrel)
    {
        return BarrelNBT.from(aircooledbarrel).durability;
    }

    public int getDamage(ItemStack aircooledbarrel)
    {
        return BarrelNBT.from(aircooledbarrel).damage;
    }

    public float getHardness(ItemStack aircooledbarrel)
    {
        return BarrelNBT.from(aircooledbarrel).hardness;
    }

    public void damageItem(ItemStack aircooledbarrel, EntityLivingBase player, int damage)
    {
        ToolHelper.damageTool(aircooledbarrel, damage, player);
    }

    @SideOnly(Side.CLIENT)
    public int getCoreColor(ItemStack aircooledbarrel)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(aircooledbarrel));
        return materials.get(0).materialTextColor;
    }

    @SideOnly(Side.CLIENT)
    public int getFaceColor(ItemStack aircooledbarrel)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(aircooledbarrel));
