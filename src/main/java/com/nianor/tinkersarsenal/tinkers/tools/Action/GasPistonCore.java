...
public class GasPistonCore extends ActionCore {

    public static boolean manual = false;

    public GasPistonCore() {
...
*/
    ...
    public float getWeight(ItemStack gaspiston)
    {
        return MechanicalNBT.from(gaspiston).weight;
    }

    public float getMech(ItemStack gaspiston)
    {
        return MechanicalNBT.from(gaspiston).mech_effectiveness;
    }

    public int getDurability(ItemStack gaspiston)
    {
        return MechanicalNBT.from(gaspiston).durability;
    }

    public int getDamage(ItemStack gaspiston)
    {
        return MechanicalNBT.from(gaspiston).damage;
    }

    public float getHardness(ItemStack gaspiston)
    {
        return MechanicalNBT.from(gaspiston).hardness;
    }

    public void damageItem(ItemStack gaspiston, EntityLivingBase player, int damage)
    {
        ToolHelper.damageTool(gaspiston, damage, player);
    }

    @SideOnly(Side.CLIENT)
    public int getCoreColor(ItemStack gaspiston)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(gaspiston));
        return materials.get(0).materialTextColor;
    }

    @SideOnly(Side.CLIENT)
    public int getFaceColor(ItemStack gaspiston)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(gaspiston));
    ...
    @SideOnly(Side.CLIENT)
    public int getMechColor(ItemStack gaspiston)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(gaspiston));
