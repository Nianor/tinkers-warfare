...
public class SingleActionCore extends ActionCore {

    public static boolean manual = true;

    public SingleActionCore() {
...
*/
    ...
    public float getWeight(ItemStack singleaction)
    {
        return MechanicalNBT.from(singleaction).weight;
    }

    public float getMech(ItemStack singleaction)
    {
        return MechanicalNBT.from(singleaction).mech_effectiveness;
    }

    public int getDurability(ItemStack singleaction)
    {
        return MechanicalNBT.from(singleaction).durability;
    }

    public int getDamage(ItemStack singleaction)
    {
        return MechanicalNBT.from(singleaction).damage;
    }

    public float getHardness(ItemStack singleaction)
    {
        return MechanicalNBT.from(singleaction).hardness;
    }

    public void damageItem(ItemStack singleaction, EntityLivingBase player, int damage)
    {
        ToolHelper.damageTool(singleaction, damage, player);
    }

    @SideOnly(Side.CLIENT)
    public int getCoreColor(ItemStack singleaction)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(singleaction));
        return materials.get(0).materialTextColor;
    }

    @SideOnly(Side.CLIENT)
    public int getFaceColor(ItemStack singleaction)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(singleaction));
    ...
    @SideOnly(Side.CLIENT)
    public int getMechColor(ItemStack singleaction)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(singleaction));
