...
public class LockingBoltCore extends ChamberCore {

    public LockingBoltCore () {
...
*/
    ...
    public float getWeight(ItemStack lockingbolt)
    {
        return MechanicalNBT.from(lockingbolt).weight;
    }

    public float getMech(ItemStack lockingbolt)
    {
        return MechanicalNBT.from(lockingbolt).mech_effectiveness;
    }

    public int getDurability(ItemStack lockingbolt)
    {
        return MechanicalNBT.from(lockingbolt).durability;
    }

    public int getDamage(ItemStack lockingbolt)
    {
        return MechanicalNBT.from(lockingbolt).damage;
    }

    public float getHardness(ItemStack lockingbolt)
    {
        return MechanicalNBT.from(lockingbolt).hardness;
    }

    public void damageItem(ItemStack lockingbolt, EntityLivingBase player, int damage)
    {
        ToolHelper.damageTool(lockingbolt, damage, player);
    }

    @SideOnly(Side.CLIENT)
    public int getCoreColor(ItemStack lockingbolt)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(lockingbolt));
        return materials.get(0).materialTextColor;
    }

    @SideOnly(Side.CLIENT)
    public int getFaceColor(ItemStack lockingbolt)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(lockingbolt));
    ...
    @SideOnly(Side.CLIENT)
    public int getMechColor(ItemStack lockingbolt)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(lockingbolt));
