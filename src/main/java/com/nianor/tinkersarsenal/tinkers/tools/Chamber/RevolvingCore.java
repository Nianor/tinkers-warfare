...
public class RevolvingCore extends ChamberCore {

    public RevolvingCore () {
...
*/
    ...
    public float getWeight(ItemStack revolving)
    {
        return MechanicalNBT.from(revolving).weight;
    }

    public float getMech(ItemStack revolving)
    {
        return MechanicalNBT.from(revolving).mech_effectiveness;
    }

    public int getDurability(ItemStack revolving)
    {
        return MechanicalNBT.from(revolving).durability;
    }

    public int getDamage(ItemStack revolving)
    {
        return MechanicalNBT.from(revolving).damage;
    }

    public float getHardness(ItemStack revolving)
    {
        return MechanicalNBT.from(revolving).hardness;
    }

    public void damageItem(ItemStack revolving, EntityLivingBase player, int damage)
    {
        ToolHelper.damageTool(revolving, damage, player);
    }

    @SideOnly(Side.CLIENT)
    public int getCoreColor(ItemStack revolving)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(revolving));
        return materials.get(0).materialTextColor;
    }

    @SideOnly(Side.CLIENT)
    public int getFaceColor(ItemStack revolving)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(revolving));
    ...
    @SideOnly(Side.CLIENT)
    public int getMechColor(ItemStack revolving)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(revolving));
