...
public class HeavyBarrelCore extends BarrelCore {

    public HeavyBarrelCore () {
        super(ArsenalPartMaterialTypes.core(CommonProxy.STURDY_TUBING), ArsenalPartMaterialTypes.face(CommonProxy.STURDY_TUBING));
    }
    @Override
    protected BarrelNBT buildTagData(List<Material> materials) {
        ...
        coreMaterials.add(CommonProxy.STURDY_TUBING.getCost());
        faceMaterials.add(CommonProxy.STURDY_TUBING.getCost());

        CoreMaterialStats sturdy_tubing = materials.get(0).getStatsOrUnknown(ArsenalMaterialTypes.CORE);
        FaceMaterialStats sturdy_tubing2 = materials.get(1).getStatsOrUnknown(ArsenalMaterialTypes.FACE);
        BarrelNBT data = new BarrelNBT();
        data.core(coreMaterials, sturdy_tubing);
        data.face(faceMaterials, sturdy_tubing2);
        return data;
    }
...
*/
    ...
    public float getWeight(ItemStack heavybarrel)
    {
        return BarrelNBT.from(heavybarrel).weight;
    }

    public float getRifling(ItemStack heavybarrel)
    {
        return BarrelNBT.from(heavybarrel).rifling;
    }

    public int getDurability(ItemStack heavybarrel)
    {
        return BarrelNBT.from(heavybarrel).durability;
    }

    public int getDamage(ItemStack heavybarrel)
    {
        return BarrelNBT.from(heavybarrel).damage;
    }

    public float getHardness(ItemStack heavybarrel)
    {
        return BarrelNBT.from(heavybarrel).hardness;
    }

    public void damageItem(ItemStack heavybarrel, EntityLivingBase player, int damage)
    {
        ToolHelper.damageTool(heavybarrel, damage, player);
    }

    @SideOnly(Side.CLIENT)
    public int getCoreColor(ItemStack heavybarrel)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(heavybarrel));
        return materials.get(0).materialTextColor;
    }

    @SideOnly(Side.CLIENT)
    public int getFaceColor(ItemStack heavybarrel)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(heavybarrel));
