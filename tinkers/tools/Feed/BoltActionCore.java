...
public class BoltActionCore extends FeedCore {

    public BoltActionCore () {
...
*/
    ...
    public float getWeight(ItemStack boltaction)
    {
        return MechanicalNBT.from(boltaction).weight;
    }

    public float getMech(ItemStack boltaction)
    {
        return MechanicalNBT.from(boltaction).mech_effectiveness;
    }

    public int getDurability(ItemStack boltaction)
    {
        return MechanicalNBT.from(boltaction).durability;
    }

    public int getDamage(ItemStack boltaction)
    {
        return MechanicalNBT.from(boltaction).damage;
    }

    public float getHardness(ItemStack boltaction)
    {
        return MechanicalNBT.from(boltaction).hardness;
    }

    public void damageItem(ItemStack boltaction, EntityLivingBase player, int damage)
    {
        ToolHelper.damageTool(boltaction, damage, player);
    }

    @SideOnly(Side.CLIENT)
    public int getCoreColor(ItemStack boltaction)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(boltaction));
        return materials.get(0).materialTextColor;
    }

    @SideOnly(Side.CLIENT)
    public int getFaceColor(ItemStack boltaction)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(boltaction));
    ...
    @SideOnly(Side.CLIENT)
    public int getMechColor(ItemStack boltaction)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(boltaction));
