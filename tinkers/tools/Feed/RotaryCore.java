...
public class RotaryCore extends FeedCore {
...
*/
    ...
    public float getWeight(ItemStack rotary)
    {
        return MechanicalNBT.from(rotary).weight;
    }

    public float getMech(ItemStack rotary)
    {
        return MechanicalNBT.from(rotary).mech_effectiveness;
    }

    public int getDurability(ItemStack rotary)
    {
        return MechanicalNBT.from(rotary).durability;
    }

    public int getDamage(ItemStack rotary)
    {
        return MechanicalNBT.from(rotary).damage;
    }

    public float getHardness(ItemStack rotary)
    {
        return MechanicalNBT.from(rotary).hardness;
    }

    public void damageItem(ItemStack rotary, EntityLivingBase player, int damage)
    {
        ToolHelper.damageTool(rotary, damage, player);
    }

    @SideOnly(Side.CLIENT)
    public int getCoreColor(ItemStack rotary)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(rotary));
        return materials.get(0).materialTextColor;
    }

    @SideOnly(Side.CLIENT)
    public int getFaceColor(ItemStack rotary)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(rotary));
    ...
    @SideOnly(Side.CLIENT)
    public int getMechColor(ItemStack rotary)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(rotary));
