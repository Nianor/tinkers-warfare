...
public class PivotingCore extends ChamberCore {
...
*/
    ...
    public float getWeight(ItemStack pivoting)
    {
        return MechanicalNBT.from(pivoting).weight;
    }

    public float getMech(ItemStack pivoting)
    {
        return MechanicalNBT.from(pivoting).mech_effectiveness;
    }

    public int getDurability(ItemStack pivoting)
    {
        return MechanicalNBT.from(pivoting).durability;
    }

    public int getDamage(ItemStack pivoting)
    {
        return MechanicalNBT.from(pivoting).damage;
    }

    public float getHardness(ItemStack pivoting)
    {
        return MechanicalNBT.from(pivoting).hardness;
    }

    public void damageItem(ItemStack pivoting, EntityLivingBase player, int damage)
    {
        ToolHelper.damageTool(pivoting, damage, player);
    }

    @SideOnly(Side.CLIENT)
    public int getCoreColor(ItemStack pivoting)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(pivoting));
        return materials.get(0).materialTextColor;
    }

    @SideOnly(Side.CLIENT)
    public int getFaceColor(ItemStack pivoting)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(pivoting));
    ...
    @SideOnly(Side.CLIENT)
    public int getMechColor(ItemStack pivoting)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(pivoting));
