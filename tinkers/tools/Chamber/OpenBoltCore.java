...
public class OpenBoltCore extends ChamberCore {
...
*/
    ...
    public float getWeight(ItemStack openbolt)
    {
        return MechanicalNBT.from(openbolt).weight;
    }

    public float getMech(ItemStack openbolt)
    {
        return MechanicalNBT.from(openbolt).mech_effectiveness;
    }

    public int getDurability(ItemStack openbolt)
    {
        return MechanicalNBT.from(openbolt).durability;
    }

    public int getDamage(ItemStack openbolt)
    {
        return MechanicalNBT.from(openbolt).damage;
    }

    public float getHardness(ItemStack openbolt)
    {
        return MechanicalNBT.from(openbolt).hardness;
    }

    public void damageItem(ItemStack openbolt, EntityLivingBase player, int damage)
    {
        ToolHelper.damageTool(openbolt, damage, player);
    }

    @SideOnly(Side.CLIENT)
    public int getCoreColor(ItemStack openbolt)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(openbolt));
        return materials.get(0).materialTextColor;
    }

    @SideOnly(Side.CLIENT)
    public int getFaceColor(ItemStack openbolt)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(openbolt));
    ...
    @SideOnly(Side.CLIENT)
    public int getMechColor(ItemStack openbolt)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(openbolt));
