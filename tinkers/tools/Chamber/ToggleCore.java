...
public class ToggleCore extends ChamberCore {
...
*/
    ...
    public float getWeight(ItemStack toggle)
    {
        return MechanicalNBT.from(toggle).weight;
    }

    public float getMech(ItemStack toggle)
    {
        return MechanicalNBT.from(toggle).mech_effectiveness;
    }

    public int getDurability(ItemStack toggle)
    {
        return MechanicalNBT.from(toggle).durability;
    }

    public int getDamage(ItemStack toggle)
    {
        return MechanicalNBT.from(toggle).damage;
    }

    public float getHardness(ItemStack toggle)
    {
        return MechanicalNBT.from(toggle).hardness;
    }

    public void damageItem(ItemStack toggle, EntityLivingBase player, int damage)
    {
        ToolHelper.damageTool(toggle, damage, player);
    }

    @SideOnly(Side.CLIENT)
    public int getCoreColor(ItemStack toggle)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(toggle));
        return materials.get(0).materialTextColor;
    }

    @SideOnly(Side.CLIENT)
    public int getFaceColor(ItemStack toggle)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(toggle));
    ...
    @SideOnly(Side.CLIENT)
    public int getMechColor(ItemStack toggle)
    {
        List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(toggle));
